  ...
  <div class>
    <div class="bg-purple-200 p-16">
      ...
      <div class="flex flex-col items-center w-full justify-center mt-5">
        <div
          ...
          :style="{backgroundImage: 'url('+uri+sermon.image_link+')'}"
        >
          <div class="text-white lg:-ml-8 -ml-5 mt-5">
            <ShareNetwork
              network="facebook"
              :url="'vivify365.org'"
              :title="sermon.title ? sermon.title : 'Vivify blog'"
    ...
    <!-- <div class="container lg:mx-auto mx-auto my-8">
      ...
      <div
        ...
        v-if="sermon.comments && sermon.comments.length == 0"
      >No comments yet</div>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      //   }
      // ],
      // blogText: `this is the sermon description`
    };
  },
  computed: {
    pageId() {
    ...
    audioLink() {
      return this.uri + this.sermon.audio_link;
    }
  },
  methods: {
    downloadAudio() {
      window.location.href = this.uri + this.sermon.audio_link;
      //   window.open(
    ...
    async getSermon() {
      ...
      await this.$store.dispatch("getSermonById", id).then(res => {
        this.sermon = res.data;
        // this.prevPost = res.prev_blog;
        // this.nextPost = res.next_blog;
        // console.log(this.uri + res.data.audio_link);
      });
    },
    async comment() {
      //   let id = this.$route.params.id;
